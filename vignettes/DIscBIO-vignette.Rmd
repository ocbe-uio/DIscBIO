...
if (OnlyExpressionFiltering==TRUE){
    ...
    sc <- FinalPreprocessing(sc,GeneFlitering="ExpF",export=FALSE)
}
```
...
if(RemovingOutliers==TRUE){
    ...
    dim(DataSet)
    colnames(DataSet)
    message("Outlier cells were removed, now you need to start from the beginning")
}
```
...
Model-based clustering assumes that the data are generated by a model and attempts to recover the original model from the data to define cellular clusters. The steps reproduced here are comparable to the ones on Section [2.1](#21-k-means-clustering).
...
```{r, eval=FALSE}
sc <- MB_Order(sc,quiet=FALSE, export=FALSE)
```
