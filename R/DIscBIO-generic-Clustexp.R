...
setMethod(f="Clustexp",
		...
		clustfun <- function(x,clustnr=20,bootnr=50,metric="pearson",do.gap=TRUE,SE.method="Tibs2001SEmax",SE.factor=.25,B.gap=50,cln=0,rseed=17000,quiet = FALSE){
			...
			if ( do.gap | cln > 0 ){
				...
				runs = 100, scaledata = FALSE, alpha = 0.001, critout = FALSE, plot = FALSE, method="euclidean", ...) {
					data <- as.matrix(data)
					...
					for (k in krange) {
						if (k > 1) {
							...
							km[[k]]$cluster))
							if (critout) cat(k, " clusters ", crit[k], "\n")
						}
					}
					if (cluster1) cluster1 <- dudahart2(data, km[[2]]$cluster, alpha = alpha)$cluster1
				...
				KmeansCBI <- function (data, krange, k = NULL, scaling = FALSE, runs = 1, criterion = "ch", method="euclidean",...) {
					if (!is.null(k)) krange <- k
					if (!identical(scaling, FALSE)) sdata <- scale(data, center = TRUE, scale = scaling)
					else sdata <- data
