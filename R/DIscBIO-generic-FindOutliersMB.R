#' @title Inference of outlier cells in Model-based clustering
...
setMethod(
    ...
    for (n in 1:max(object@MBclusters$clusterid)) {
        if (sum(object@MBclusters$clusterid == n) == 1) {
        cprobs <-
            append(cprobs, .5)
        names(cprobs)[length(cprobs)] <-
            names(object@MBclusters$clusterid)[object@MBclusters$clusterid == n]
        next
        }
    ...
    if (length(out) > 0) {
        ...
        for (i in 1:length(out)) {
            if (length(n) > 1) {
            o   <-
                order(apply(cbind(m, 1:dim(m)[1]), 1, function(x)
    ...
    if (plot) {
        ...
        if (i == 1)
            ...
            barplot(
            y,
            ylim = c(0, max(-log10(x + 2.2e-16)) * 1.1),
            col = fcol[i],
            border = fcol[i],
            names.arg = FALSE,
            ylab = "-log10prob"
